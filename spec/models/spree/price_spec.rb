...
describe Spree::Price do
  it 'can build a new sale by factory' do
    price = build(:price)
    sale_price = price.new_sale({ value: 15.99 })
  ...
  it 'can put a price on a percent-off sale' do
    price = create(:price)
    price.put_on_sale(value: 0.2, calculator_type: Spree::Calculator::PercentOffSalePriceCalculator.new)
